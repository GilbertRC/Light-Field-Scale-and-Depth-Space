...
% Outputs:
% -L_1 : The Lisad-2 space.
function L_2 = construct_Lisad_2_space(epi_horizontal, nOctave, nLayer, scale_0, nAngle)
nScale = nOctave*nLayer;
nX = size(epi_horizontal,2);
nU = size(epi_horizontal,1);
uC = ceil((nU+1)/2);
L_2 = zeros(nX,nScale,nAngle,'double');
for scale_idx = 1:nScale
    for alpha_idx = 1:nAngle
        ...
        % construct Lisad-2 space
        ray_gaussian_kernal_diff2 = scale^2 * imfilter(ray_gaussian_kernal,[1,-2,1],'symmetric');
        cur_L_2 = imfilter(epi_horizontal,ray_gaussian_kernal_diff2,'symmetric');
        L_2(:,scale_idx,alpha_idx) = cur_L_2(uC,:);
